...
gen1 = pqr.engine() # equivalent to "gen1 = engine(True)"
...
gen2 = pqr.engine(False) # If we forget to pass False, it only wastes time, it's not detrimental
...
x1 = dist.GetSample(n, gen1)
y1 = dist.GetSample(n, gen2)
...
x2 = dist.GetSample(n, gen1)
y2 = dist.GetSample(n, gen2)
...
nGen = 4
subFigIndex = 1
gen1.Seed_FromFile("test.seed")
seedList = gen1.GetState_JumpVec(nGen)
...
for i in range(nGen):
	for j in range(i + 1, nGen):
		...
		# apples to apples
		gen1.Seed_FromString(seedList[i])
		gen2.Seed_FromString(seedList[j])
		x = dist.GetSample(n, gen1)
		y = dist.GetSample(n, gen2)
...
plt.hist(dist.GetSample(nHist, gen3), bins=fdBins, normed=1, histtype="step")
